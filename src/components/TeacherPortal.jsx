...
import { Send, Clock, CheckCircle, BookOpen, Users, Plus, X, AlertCircle, Radio, BarChart, Download, MessageSquare } from 'lucide-react';
...
export default function TeacherPortal({ user, addToast, assignments, setAssignments, attendance, setAttendance }) {
  ...
  const sendBroadcast = (e) => {
    ...
    const targetText = broadcastForm.target === 'All' ? 'All Divisions' : `Division ${broadcastForm.target}`;
    const studentCount = broadcastForm.target === 'C' ? 423 : 423 * 7; // Mock count
    addToast({ type: 'success', title: '📡 Broadcast Sent', msg: `Broadcast sent to ${targetText} — ${studentCount} students notified` });
